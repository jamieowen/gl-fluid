...
window.document.write(
	'<input id="scale" type="range" min="0.0" max="2.0" step="0.001">scale</input>' +
	'<input id="strength" type="range" min="0.0" max="2.0" step="0.001">strength</input>' +
	'<br/><canvas id="example" width="512" height="512"></canvas>'
);
...
window.onload = function(){
	...
	strengthInput.value = scale;
	strengthInput.onchange = function(){
	...
	raf( function render(){
		...
		if( down ){
			...
			if( pos.x < 256 && pos.y < 256 ){
				...
				scalePos = [ fluid2d._shape[0] / 256, fluid2d._shape[1] / 256 ]; // for testing diff sizes
				fluid2d.droplet( pos.x * scalePos[0], pos.y * scalePos[1], scale, strength, dropTex );

			}else
			if( pos.x < 512 && pos.y < 256 ){
				var p = { x : pos.x - 256 , y: pos.y };
				scalePos = [ fluid1d._shape[0] / 256, fluid1d._shape[1] / 256 ]; // for testing diff sizes
				console.log( '>>>', p.x * scalePos[0], p.y * scalePos[1] );
				fluid1d.droplet( p.x * scalePos[0], p.y * scalePos[1], scale, strength, dropTex );
			}
		}else{
		...
		fluid2d.texture().bind(0);
		gl.viewport( 0, 256, 256, 256 );
		drawTriangle(gl);

		fluid1d.texture().bind(0);
